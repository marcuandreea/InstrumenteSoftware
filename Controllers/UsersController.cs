...
namespace mvc.Controllers
{
    public class UsersController : Controller
    {
        ...
        public UsersController(UserManager<Users> userManager, IUserService userService, RoleManager<IdentityRole> roleManager) {
        ...
        [Authorize(Roles = "Admin,Abonat,Vizitator")]
        public async Task<IActionResult> Settings(string id, [Bind("Id,UserName,Email,PasswordHash,Tip_utilizator,ID_abonament")] Users user, IFormFile? photoPathFile, string? currentPassword, string? newPassword, string? confirmPassword)
        {
            if (id != user.Id)
            {
        ...
        [ValidateAntiForgeryToken]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> Create([Bind("UserName,Email,PasswordHash,Tip_utilizator,ID_abonament,photoPath")] Users user, IFormFile? photoPathFile)
        {
            if (photoPathFile != null && photoPathFile.Length > 0)
            {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    // Add the user and assign the Tip_utilizator as a role
                    await _userService.AddUserAsync(user, user.Tip_utilizator);
        ...
        [ValidateAntiForgeryToken]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> Edit(string id, [Bind("Id,UserName,Email,PasswordHash,Tip_utilizator,ID_abonament,photoPath")] Users user, string? newPassword, IFormFile? photoPathFile)
        {
            if (id != user.Id)
            {
                return BadRequest("User ID mismatch.");
            ...
            if (user.ID_abonament != 0 && user.ID_abonament != existingUser.ID_abonament)
            {
                existingUser.ID_abonament = user.ID_abonament;
            }

            if (!string.IsNullOrEmpty(user.Tip_utilizator) && user.Tip_utilizator != existingUser.Tip_utilizator)
            {
                existingUser.Tip_utilizator = user.Tip_utilizator;
            }
